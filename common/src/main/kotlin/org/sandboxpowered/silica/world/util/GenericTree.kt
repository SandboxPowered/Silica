 ...
 */
@Suppress("unused")
open class GenericTree<T : GenericTree<T, V>, V : Any>(
    x: Int, y: Int, z: Int,
    size: Int
) : Poolable {
    protected var bounds = Bounds().set(x, y, z, size)
    protected var nodes: MutableList<T?> = ArrayList(8)
    protected var containers: MutableList<V?> = ArrayList(8)
    protected lateinit var default: V
    var treeDepth = 0
        private set
    protected var parent: T? = null
    protected lateinit var otPool: ObjectPool<T>
    ...
    /**
     * Sets the target position to [value].
    ...
    operator fun set(
        ...
        internalSet(x, y, z, value)
    }

    /**
     * @return the previous state at given position
     */
    protected open fun internalSet(
        x: Int, y: Int, z: Int,
        value: V
    ): V {
        ...
        val n = nodes[index]
        val old = if (n != null) {
            val old = n.internalSet(x, y, z, value)
            if (n.shouldShrink()) {
            ...
            if (bounds.size > 2) {
                if ((containers[index] ?: default) != value) {
                    this.split(index)
                    nodes[index]!!.internalSet(x, y, z, value)
                } else value
            } else {
                val old = this.containers[index] ?: this.default
                this.containers[index] = if (value != this.default) value else null
                old
            }
        }
        return old
     ...
     */
    protected fun shouldShrink(): Boolean {
    ...
    operator fun get(
        ...
        return if (index == OUTSIDE || this.nodes[index] == null) this as T
    ...
    override fun toString() = "${this.javaClass}(depth=$treeDepth, $bounds)"
